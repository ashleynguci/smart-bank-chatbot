...
class InvoiceParser:
    ...
    REQUIRED_FIELDS = ['total_amount', 'vendor_name', 'iban']
    ...
    def _process_pdf_with_gemini(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            - vendor_name
            - vendor_address
            - business_id ("Y-tunnus", if in Finland)
            - account_number
            - bic
            - iban
            - reference_number
...
class FieldValidator:
    ...
    Missing values are considered non-valid.
    """
    ...
    def invoice_number(self, value: Any) -> tuple[bool, Optional[str], Optional[str]]:
        ...
        """
        if value is None:
            return False, 'Value missing', None
    ...
    def business_id(self, value: Any) -> tuple[bool, Optional[str], Optional[str]]:
        """
        Validate business ID. If in Finland, it must be a valid Finnish business ID
        ("y-tunnus", in Finnish).
        """
        if value is None:
            return False, 'Value missing', None
        ...
        try:
            ...
            # Check if it matches the Finnish business ID format (7 digits, hyphen, check digit)
            if not re.match(r'^\d{7}-\d$', value_str):
                return False, "Invalid Finnish business ID format. Expected format: 1234567-8", None

            # Extract the 7-digit part and the check digit
            digits = value_str.split('-')[0]
            check_digit = int(value_str.split('-')[1])

            # Calculate the check digit
            weights = [7, 9, 10, 5, 8, 4, 2]
            sum_product = sum(int(digit) * weight for digit, weight in zip(digits, weights))
            ...
            # If calculated check digit is 10, it's invalid
            if calculated_check_digit == 10:
                return False, "Invalid Finnish business ID: check digit calculation resulted in 10", None

            # Verify the check digit
            if check_digit != calculated_check_digit:
                return False, f"Invalid Finnish business ID: check digit should be {calculated_check_digit}", None

            return True, None, value_str

        except Exception as e:
            return False, f"Invalid business ID: {e}", None
