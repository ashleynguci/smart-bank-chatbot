# Created by following this tutorial: https://python.langchain.com/docs/tutorials/rag/
# Document loader (Allows different types in addition to websites, e.g. json, PDF, csv, etc.): https://python.langchain.com/docs/integrations/document_loaders/
# Sitemap scraping: https://python.langchain.com/docs/integrations/document_loaders/sitemap/
# Sources: https://python.langchain.com/docs/how_to/qa_sources/
...
CHUNK_SIZE = 3000  # Maximum size of a chunk in characters
CHUNK_OVERLAP = 200  # Overlap between chunks in characters
RETRIEVED_DOCS_AMOUNT = 10 # Number of documents to retrieve for each query. The more documents, the more spent tokens, but also more accurate responses, and the more context for the LLM to use.
...
def exclude_footer(name, attrs):
    # Exclude <footer class="footer">, include everything else
    return not (name == "footer" and "footer" in attrs.get("class", []))
...
agent_executor = create_react_agent(
    ...
    [retrieve], 
    checkpointer=memory, 
    prompt="""You are a chat assistant embedded into the Nordea internet bank. 
    You have a retrieve tool that can be used to refer to relevant banking, loan and service information from the Nordea website. 
    Use it to answer questions related to banking. Cite the source links at the end of the message.
    ...
    You must use the ResponseFormatter tool as a last step to structure your final response JSON. 
    
    """,
    response_format=ResponseFormatter,
    )

input_message = "Miten opintolainan korko määräytyy?"
...
for event in agent_executor.stream(
  ...
  last_event = event  # Keep updating with the latest event
  last_event["messages"][-1].pretty_print()

if last_event:
  # The structured_response is already a Pydantic model (ResponseFormatter)
  # To get the JSON/dict, use .dict() or .model_dump()
  ...
  # If you want to return it from a FastAPI endpoint, just: return response_json
